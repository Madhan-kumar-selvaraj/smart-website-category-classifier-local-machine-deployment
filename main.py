...
@app.get("/")
def read_root():
    return {return {"Hi!": "Welcome to the website classifier API. By using this API one can get to know the catergory of the website URL and the most commonly used words. It can able to learn from your input that's why it is knows as smart classifier", "API's Rule book": "You can get to know the category of this website and also you can add your own category for your website. It will help our API to learn from your data and the result will be more accurate", "Rule 1" : "To get to know the category, send the request in this format 'http://127.0.0.1:8000/(url)'. For example if the url is 'https://news.google.co.in' then replace the '/' with '*' and send the request like this 'http://127.0.0.1:8000/https:**news.google.co.in'", "Rule 2" : "You can add your own category in this format http://127.0.0.1:8000/(url)?category=(category). For example if your url is 'https://www.mdpi.com/journal/agriculture' and category is 'agriculture'. request format is 'http://127.0.0.1:8000/https:**www.mdpi.com*journal*agriculture?category=agriculture'"}}
